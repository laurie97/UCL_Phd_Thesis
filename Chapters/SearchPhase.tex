...
with initial parameters seeded initially from a configuration file and then from the previous fit.
...
\item \textbf{The window size:}\\
  As described, symmetric windows around the window centre are used, with a width defined by their window half-width ($wHW$) parameter.\vspace{0.5em}
...
\item \textbf{Number of windows with Wilks' \mbox{$p$-value} $<$ 0.1 must be $\leq$ \\10 (low mass) / 15(high mass)}:\\
  ...
  to provide an adequate description of the data, as described in \ref{sec:bkg-bkg_wilks}. 
...
%  \includegraphics[width=0.45\linewidth, angle=0,page=1]{figs/Dibjet/LowMass/FitStudy_min566/windowSel_dataLike_5para.pdf}
%}
...
\item If the tests are passed the background estimation is used.
\end{enumerate}
...
shows the results of the SWiFt search phase procedure
for the 5 parameter fit function for the window half-widths of 10 and 16.
...
Therefore, in in the case of the window half-width of 14 and 16 the region exclusion is applied.
The region excluded is 705-834 GeV, derived by adding one bin on the low mass side of the excess region identified by the bumpHunter algorithm.
Figure~\ref{fig:bhFit_lm_corrFitCR_dataLike_inj_Zprimebb800_xsFactor1_removedWindow} shows the SWiFt search phase procedure
...
Figure~\ref{fig:windowSel_Zprimebb800_xsFactor1} shows the fit quality measures used in window selection procedure after the region exclusion of 705-834 GeV is applied,
for a window half-width of 14 and 16 for the 5 parameter fit function.
Only two window half-widths are considered these are the only windows that had a significant enough $p$-value to trigger the region exclusion procedure in
Figure~\ref{fig:bhFit_lm_corrFitCR_dataLike_inj_Zprimebb800_xsFactor1}.
...
a $Z'$ boson with a mass of 800 GeV at the nominal cross-section using the 5 parameter fit function.
The bumpHunter $p$-value assigned after region exclusion is applied is $<$ 0.001 using 10,000 pseudo-experiments,
which represents a excess that is greater than a 3~\sigma deviation.
...
the cross-section needs to be increased to 2 and 3 times respectively.
The larger cross-section for the 600 GeV is required due to the fact that it is close to the start of the dijet mass spectrum,
and thus has no side-band to restrict the fit.
...
These conclusions are important in driving the choice of the window selection procedure in the high-mass case, which is described in Section~\ref{sec:windowSel}.
...
For the full \lm{} data-set a window half-width is chosen using the window selection procedure outlined in Section~\ref{sec:windowSel}.
The SWiFt background estimation is performed for the full data-set with a window half-width range of 16 to 10 for the 5 parameter fit function.
Figure~\ref{fig:windowSel_unblind} shows the two fit quality measures used in the window and fit function selection,
the global $\chi^2$ $p$-value and the number of windows with a Wilks' $p$-value $<$ 0.1.
The 5 parameter fit function with a window half-width of 16 is selected as it is the
widest window that passes the fit quality criteria outlined in Section~\ref{sec:windowSel}.
Figure~\ref{fig:windowSel_unblind_just16} shows the Wilks' $p$-value and $\chi^2$ $p$-value for fits in each of the windows
as a function of the window centre, further showing that the SWiFt procedure has found a good background estimation for this window half-width.
...
\centering
\subcaptionbox{Global $\chi^{2}$ \mbox{$p$-value}} {
  \includegraphics[width=0.43\linewidth, angle=0,page=6]{figs/Dibjet/LowMass/FitStudy_min566/windowSel_unblind.pdf}
}\hspace{-8mm}
\subcaptionbox{Number of window fits with\\ Wilks' \mbox{$p$-value} $<$ 0.1} {
  \includegraphics[width=0.43\linewidth, angle=0,page=8]{figs/Dibjet/LowMass/FitStudy_min566/windowSel_unblind.pdf}
}
\vspace{8pt}
...
fitted to using the SWiFt background estimation procedure using the 5 parameter dijet fit function and a window half-width of 16.
The BumpHunter algorithm has identified the most discrepant excess indicated in the figure using vertical blue lines,
The BumpHunter \mbox{$p$-value} is 0.603, which has been calculated using 10,000 pseudo-experiments.
This $p$-value shows that no significant excess is found 
...
\caption{\label{fig:bhFit_lm_unblind}
   The dijet mass spectrum (\mjj) of the  full \lm{} data-set fitted to using the SWiFt background estimation procedure

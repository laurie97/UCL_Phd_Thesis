...
\end{equation}
where $\Like_{\text{Nom}}$ and $\Like_{\text{Alt}}$ are the maximised likelihoods of the nominal and alternate function respectively,
using the definition of Likelihood given in Equation~\ref{eqn:bkg-like}.

Using Wilks' theorem it is known that for nested functions, such as the dijet fit functions,
that the Wilks' test statistic will follow a $\chi^2$ distribution with 1 degree of freedom~\cite{dibjet-wilks}.
...
The first fit test is to demonstrate that the dijet fit functions are able to describe the dijet mass spectra.
...
\item \textbf{Number of windows with Wilks' \mbox{$p$-value} $<$ 0.1 must be $\leq$ \\10 (low mass) / 15(high mass)}:\\
  ...
  to provide an adequate description of the data, as described in \ref{sec:bkg-full_bkg-bkg_wilks}. 
...
However this configuration is not used as it it is less sensitivite to signal than the 5 parameter fit with wider windows, as shown will be shown in Section~\ref{sec:bkg-full_signalInj}.

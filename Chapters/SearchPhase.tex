...
As described above, the SWiFt procedure involves breaking the spectrum up into smaller windows to perform a window fit in each window.
Hence, there are two key input parameters of the SWiFt fit procedure.
\noindent
They are:
\begin{enumerate}[leftmargin=*]
\item \textbf{The window size:}\\
  As described above the window width is defined as symmetric windows are used, defined by their window-half-width (wHW).\vspace{0.5em}
...
To select the optimal SWiFt configuration, we will use an predefined iterative window selection procedure which can be run on the final data-set when the analysis is unblinded.
...
\caption{\label{fig:windowSel_dataLike}
  The cumulative probability of the global $\chi^{2}$ $p$-value, %global Kolmogorov-Smirnov $p$-value
...
\caption{\label{fig:windowSel_subset}
  ...
  It shows the global $\chi^{2}$ $p$-value, %global Kolmogorov-Smirnov $p$-value
  and number of window fits with Wilks' $p$-value $<$ 0.1 for SWiFt background estimate,
...
To provide a more quantitative statement on if spurious signal is observed,
...
In addition Table~\ref{tab:bumpH_lm_spuriousSignal} shows the percentage of seeds that have a {\sc BumpHunter} $p$-value less than %0.1,
...
%For the 5 parameter fit with a window-half-width of 16, we see that there are more than 10\% of events with a
%{\sc BumpHunter} $p$-value less than 0.1, which suggest a slight fit discrepancy.
%However, we see that that only 0.9\% of seeds have {\sc BumpHunter} $p$-value of less than 0.01, which is within expectations.
%In addition, as shown in Section~\ref{sec:lowmass_windowSel},
%in most cases our window selection procedure would not select the 5 parameter fit function with a window-half-width of 16.
...
%\begin{tabular}{|c|c||c|c|c|c|}
%  \hline
%  wHW  &  Mass Range of Excess [GeV]  &  BumpHunter $p$-value \\
%  \hline
%  16   &  566 - 611 & $<$ 0.001 \\
%  14   &  566 - 611 & $<$ 0.001 \\
%  12   &  566 - 611 & $<$ 0.001 \\
...
\captionsetup[subfigure]{aboveskip=0pt,justification=centering}
\centering
\subcaptionbox{5 parameter-fit, wHW = 12\\No Window Exclusion
  \label{fig:bhFit_lm_corrFitCR_dataLike_inj_Zprimebb600_xsFactor1}} {
  \includegraphics[width=0.45\linewidth, angle=0]{figs/Dibjet/LowMass/FitStudy/bhFit_corrFitCR_dataLike_v13_5para_low12_high12_inj_Zprimebb600_xsFactor1.pdf}
}
\subcaptionbox{5 parameter-fit, wHW = 12\\Window Excluded 566-611 GeV
...
Next, we can consider the $Z'$ of mass 800 GeV.
...
\captionsetup[subfigure]{aboveskip=0pt,justification=centering}
\centering
\subcaptionbox{5 parameter-fit, wHW = 10\\No Exclusion Region} {
...
\subcaptionbox{5 parameter-fit, wHW = 10\\Exclusion Region 705-807 GeV } {
...
\subcaptionbox{5 parameter-fit, wHW = 10\\Exclusion Region 705-834 GeV} {

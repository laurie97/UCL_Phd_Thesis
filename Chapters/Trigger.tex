...
the first level trigger (L1) and the higher level trigger (HLT)~\cite{trig-run2_trigger,trig-run2_proc}.
Figure~\ref{fig:trig-trigger_schem} shows a schematic outlining the trigger system used in Run-2
\footnote{\ The period of data-taking starting in 2015 and scheduled to continue until the end 2018 is known as Run-2.}~\cite{trig-run2_triggerPerf}. 
...
In Run-2 the jet triggers are used at both the L1 and HLT level~\cite{trig-L1calo,trig-bTrig_desc}.
...
It is required that events are in a $b$-jet trigger aware Good Run List (GRL),
where a GRL is list of events that pass certain data-quality requirements.
A further discussion of GRLs is in Section~\ref{sec:evt-datasets} and 
Section~\ref{sec:trig-grl} describes the $b$-jet trigger aware GRL and the motivations for its use.
...
\begin{table}[!htb]
  ...
  %\vspace{pt}
  \vspace{-0.5em}
  \caption[A table summarising the three epochs of data, showing the integrated luminosity after a $b$-jet trigger GRL is applied and the
    effect of not finding a \textit{xPrmVtx} primary vertex. ]
          {A table summarising the three epochs of data, showing the integrated luminosity after a $b$-jet trigger GRL is applied and the
...
%\begin{figure}[!ht]
  \begin{center}
    \captionsetup[subfigure]{aboveskip=0pt,justification=centering}
    \subcaptionbox{Jet-\pT{},\\ \textit{xPrmVtx} vertex found}{\includegraphics[width=0.45\linewidth, angle=0]{figs/Trigger/Epoch1_noGRL_eff_PVFound_jetPt.pdf}}
    \subcaptionbox{$z_{\,\text{bs}}^{\text{\,online}}$,\\ \textit{xPrmVtx} vertex found}{\includegraphics[width=0.45\linewidth, angle=0]{figs/Trigger/Epoch1_noGRL_eff_PVFound_bs_online_vz.pdf}}
  \end{center}
  \vspace{-1em}
...
\begin{center}
  \captionsetup[subfigure]{aboveskip=0pt,justification=centering}
  \subcaptionbox{Jet-\pT}{\includegraphics[width=0.47\linewidth, angle=0]{figs/Trigger/Epoch3_GRL_bslt2mm_eff_jetPt.pdf}}
  \subcaptionbox{Jet-$\eta$}{\includegraphics[width=0.47\linewidth, angle=0]{figs/Trigger/Epoch3_GRL_bslt2mm_eff_jetEta.pdf}} \\
  \subcaptionbox{$z_{\,\text{bs}}^{\text{\,online}}$}{\includegraphics[width=0.47\linewidth, angle=0]{figs/Trigger/Epoch3_GRL_bslt2mm_eff_bs_online_vz.pdf}}
  \subcaptionbox{Vertex Class}{\includegraphics[width=0.47\linewidth, angle=0]{figs/Trigger/Epoch3_GRL_bslt2mm_eff_vtxClass.pdf}}
\end{center}
\vspace{-1em}

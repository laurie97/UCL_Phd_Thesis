...
The LHC has collecting 13~TeV $pp$ collision data since January 2015 and is scheduled to continue until 2038~\cite{fut-lhc-shedule}.
...
It is expected that the integrated luminosity of $pp$ collision data collected by the end of 2022 will be $\sim$300~\ifb{}.
...
The High-Luminosity LHC is expected to collect 13~TeV $pp$ collision data with an integrated luminosity of $\sim$3000~\ifb{} by the end of 2038.

Table~\ref{tab:fut-lumi} summarises the integrated luminosity of the data-sets used by di-$b$-jet searches
at ATLAS and the expected integrated luminosities at key points in the LHC schedule.
...
Therefore, it is important to investiage other techniques to increase the sensitivity on a shorter time-scale.
...
Table~\ref{tab:fut-cmsComp} shows a comparison of the 95\% credibility-level observed upper mass limits set
by the \summer{} data-set analysis and the CMS di-$b$-jet search,
where the upper mass limit represents the highest mass excluded by the analysis on the two benchmark models considered.
...
The improvement from combining categories is smaller for the $b^*$ as the $\geq1~b$-tag category is already used by the \summer{} analysis.
...
Measurements of the offline $b$-tagging efficiency measurements, described in Section~\ref{sec:obj-bjets_calib},
have similar sources of systematics uncertainties.
Techniques have been developed to reduce the systematic uncertainties in the offline $b$-tagging efficiency measurements~\cite{obj-bjets_calib_tech,obj-bjets_calib_plots};
such as a Boosted Decision Tree (BDT) to increase the $b$-jet purity of the selected jets
and the inclusion of di-lepton $t\bar{t}$ events containing two electrons or two muons to increase the number of di-lepton $t\bar{t}$ events.
...
This would allow for a combined offline plus $b$-jet trigger efficiency measurement using
...
To reduce dependance of the search phase on any signal model, a large range of signal widths can be considered. 
A similar approach has been used in searches for resonances decaying into a pair of photons at ATLAS~\cite{fut-diphoton}.

...
and how many proposed BSM models predict resonances decaying to one or two $b$-quarks.
...
\item\textbf{Weak Force:}
  ...
  This feature will prove important for identifying the presence of $b$-quarks at the ATLAS detector, which will be discussed in Section~\ref{sec:obj-bjets}.
...
\item\textbf{Flavour Preference:}\\
  ...
  However, Figure~\ref{fig:theo-qcd_pdf} shows the heavy flavour quarks are supressed in
  the PDFs relative to the other partons.
  Therefore, dijet events will be dominated by jets initated by gluons or light-quarks ($u$, $d$ or $s$).
...
I will then discuss some proposed models that motivate the analyses shown in this thesis.
...
However, these contributions are found to be of the order $\delta m_H^2 \sim \frac{1}{16\pi^2} M_{Plank}^2$,
 much larger than the observed mass of the Higgs boson.
...
which makes us confident that there must exist physics Beyond the Standard Model.
...
Two such models that predict resonances at the TeV scale with preferential decays to $b$-quarks
are discussed below.
...
is well motivated.
However, it is important to note that the prior expectation in any specific model of BSM is quite low.
This is because there are many BSM theories proposed
which can explain some of the problems explained in the previous section and
there is little evidence to prefer one model over another.

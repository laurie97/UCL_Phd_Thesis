...
\item\textbf{Electro-magnetic (EM) Force:}
  ...
  %% Note to laurie. alpha_{EM} ~ 1/120 @ mass of W boson %\vspace{1em}
...
%%\subsection{OLD VERSION : Renormalisation and the Running of $\alpha_S$}
...
The first step of the procedure, known as regularisation, separates the divergent and non-divergent terms of the cross-section calculation.
One method of regularisation introduces an arbitrarily large upper limit, $\Lambda$, to the energy that the internal gluon/quark can have
such that cross-section calculations involving an integral over all phase space no longer contain divergent terms,
however the cross-section will now depend on some unknown and large parameter $\Lambda$.
%known as the \textit{`cut-off mass scale'}
%This step removes the divergences, but has 
%There is now a finite range of energy values that the internal quark or gluon can take and hence divergences no longer appear in cross-section calculations.
%However, we have introduced some unknown and large parameter $\Lambda$.

In the second step of the procedure, known as renormalisation,
the parameters of QCD are redefined such that $\Lambda$ is absorbed into the redefined parameters.
To ensure that the parameters maintain a physical dimension in this process
...
and by setting $\mu_R = Q$ we ensure that higher-order contributions cannot become large due to a dependence on $\mu_R$.
...
\begin{figure}[!htb]
  \begin{center}
    \includegraphics[width=1\linewidth, angle=0]{figs/Theory/qcd_running.pdf}
  \end{center}
...
Finally, it should be noted that the above description of QCD dijet production is not complete;
...
It is a clear inadequacy of the Standard Model that there is no description of gravity.
...
It is assumed that dark matter interacts through gravity
...
Furthermore, it is assumed the dark matter couples to the Standard Model.
...
\begin{figure}[!hbt]
  \begin{center}
    \includegraphics[width=0.8\linewidth, angle=0]{figs/Theory/bsm_zprime.png}
  \end{center}
...
It is worth noting in the models considered that the $Z'$ boson does not preferentially decay to $b$-quarks, but rather with similar branching ratio as the other quarks.
...
In the regime that the mass of the $b^{*}$ quark is much larger than the sum of the masses of the \mbox{top-quark} and the $W$ boson,
...
\begin{figure}[!hbt]
  \begin{center}
    \includegraphics[width=0.8\linewidth, angle=0]{figs/Theory/bsm_bstar.png}
  \end{center}

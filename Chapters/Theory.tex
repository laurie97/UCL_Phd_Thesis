...
and proposed BSM models that predict resonances decaying to one or two $b$-quarks.
...
\item\textbf{Leptons:}
  ...
  hence are neglible compared the other particles of the Standard Model.
...
\item\textbf{Weak Force:}
  ...
  allowing for interactions that change generation of the quark's flavour.
...
Firstly that the size of the coupling constant, $\alpha_S$, is generally large compared to $\alpha_{EM} \sim 1/137$;
...
The second element is the Parton Density Functions (PDFs), $f_i(x_i)$, which describes the number density of
...
%  \sigma(p_1p_2\to q/g_k q/g_l) = \int dx_1 dx_2 f_1(x_1,\mu^2_F)f_2(x_2\mu^2_F) \sigma(q/g_1,q/g_2, \alpha_s(\mu^2_R),Q^2/\mu^2_F,Q^2/\mu^2_R)
  \sigma_{had} = \sum_{a,b,i,j} \int dx_i dx_j f_i(x_i,Q^2)f_j(x_j,Q^2) \hat{\sigma}(p_i, p_j\to p_k p_l)
\end{equation}
...
The first is the invariant mass of the outgoing partons, $m_{kl}$, which is given in terms of the four-momentum of the two partons by;
\begin{equation}
  m_{kl}^2 = (p^\mu_k + p^\mu_l)^2  
\end{equation}
\noindent
Then there are two related angular variables, $y^*$ and $\theta^*$,
defined in terms of $y_k$, the rapidity of the outgoing parton $p_k$;
\begin{equation}
  y^* = (\frac{y_k - y_l}{2}),
\end{equation}
\begin{equation}
...
outgoing partons $i$ and $j$ is given in terms of the variables $\theta^*$ and $m_{kl}$~\cite{theo-dijet_harris};
\begin{equation}
...
However, QCD interactions within the proton mean that a gluons can be emitted from the valence quarks
...
As QCD is not perturbative in the proton due to the large $\alpha_S$ the PDFs cannot be calculated directly.
...
however, the higher-order QCD effects (which are non-perturbative) have smeared this peak to what is observed.
...
\item\textbf{Behaviour with respect to $y^*$ :}\\
  ...
  we see that there is a $1/\hat{t}$ term, which originates from the t-channel Feynman diagrm.
  This term will become large when $\cos{\theta^*} \to 1$
  causing a larger dijet cross-section at large values of $\cos{\theta^*}$ and $y^*$.
  \vspace{0.5em}

\item\textbf{Flavour Preference:}\\
  Table~\ref{tab:theo-qcd_dijet_s} shows that all the processes that would produce heavy flavour quarks ($c$ or $b$),
...
This results in the unique topology when top-quark pair production occurs, known as $t\bar{t}$ events, which is often exploited by analyses.
...
that any new physics, and thus the resonances, may be found at the TeV scale.

Two such models that predict resonances at the TeV
scale with preferential decays to $b$-quarks
are discussed below.
...
\begin{figure}[!hbt]
  ...
  \end{center}
  \caption{The leading-order Feynman of the process $q\bar{q} \to Z' \to b\bar{b}$.}
...
\begin{figure}[!hbt]
  ...
  \end{center}
  \caption{The leading-order Feynman of the process $bg \to b^* \to bg$}

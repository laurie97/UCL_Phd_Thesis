...
and is used as the background template in the limit setting phase for the \lm{} data-set analysis.
A signal subtracted background estimation is created for each generated mass point.

The signal subtracted background is used as it provides a simple method of using the results of the signal plus background fit
...
It is beneficial for related analyses at ATLAS to utilise similar background estimation techniques as this
increases reliability because the technique is independently validated multiple times,
...
this is especially notable in the case of a SSM $Z'$ boson at 600 GeV.
...
For the \lm{} data-set analysis, limits are set on cross-section times branching ratio ($\sigma\,\text{x}\,\mathit{BR}$)
...
even though different trigger and event selections are used, the \hm{} data-set analysis result is not presented in this thesis~\footnote{Details
...
Overlaid are theoretical predictions of $\sigma\,\text{x}\,\text{BR}$ for the
...
and can be seen clearly in Figure~\ref{fig:evt-lm_acc}(a).
...
\begin{figure}[!ht]
  ...
  \caption
      [95\% credibility level upper limits on
        cross-section times branching ratio ($\sigma\,\text{x}\,\mathit{BR}$)

...
and the BSM physics model is described by a dijet mass signal template, normalised such that $N_\text{sig}$ di-$b$-jet events $^{1}$ are produced.  %%%%% This is a footnote reference. Be careful!!!!
This signal plus background hypothesis is denoted by the symbol $H_{\text{sig}}$.
...
For the hypothesis, $H_{\text{sig}}$, the probability of producing the data is known as the likelihood.
In each dijet mass bin, labelled by the index~$i$, the hypothesis $H_{\text{sig}}$ predicts
$s_i(N_\text{sig})$ signal events, $b_i$ background events and $n_i$ events were observed in data.
Therefore, by only considering statistical uncertainties, the likelihood for a given value of $N_\text{sig}$ can be calculated using Poisson probabilities:
\begin{equation}
  \Like (N_\text{sig},D) = P(D \mid N_\text{sig}) =  \Pi_i \left(\frac{(s_i(N_\text{sig})+b_i)^{n_i}~e^{-(s_i(N_\text{sig})+b_i)}}{n_i!}\right)
  \label{eqn:lim-like}
\end{equation}
...
\end{equation}
to obtain the probability density function of $N_\text{sig}$ given the observed dijet mass spectrum,
\begin{equation}
  P(N_\text{sig} \mid D) = \frac{ P(D \mid N_\text{sig}) \, \Pi( N_\text{sig} ) }{ \Pi( D ) }
...
$H_{\text{sig}}$ for any particular value of $N_\text{sig}$.
The $\Pi( N_\text{sig} )$ term in the posterior %Equation~\ref{eq:lim-conf_statOnly}
is called the signal prior
and gives the probability density of $N_\text{sig}$ before the experiment took place.
A prior flat with respect to $N_\text{sig}$ is chosen
\footnote{Flat from $N_\text{sig}$ = 0 to the value of $N_\text{sig}$ where the
likelihood has fallen to $10^{-5}$ of the optimal likelihood value.}
which represents ignorance to the size of the signal.
The $\Pi(D)$ term does not depend on $N_\text{sig}$ and as such can be considered as a normalisation term.
...
\begin{equation}
  \Like (N_\text{sig},D,\vec{\theta}) = P(D \mid N_\text{sig}, \vec{\theta} ) =  \Pi_i \left( \frac{[s_i(N_\text{sig},\vec{\theta})+b_i(\vec{\theta})]^{n_i}~e^{-[s_i(N_\text{sig},\vec{\theta})+b_i(\vec{\theta})]}}{n_i!} \right)
\end{equation}
...
one obtains the posterior for $N_\text{sig}$ that accounts for systematic uncertainties
\begin{equation}
  P(N_\text{sig} \mid D) \propto \int d \vec{\theta} \, \Like (N_\text{sig}, D, \vec{\theta} ) \, \Pi( N_\text{sig} )  \, \Pi(\vec{\theta})
  \label{eq:lim-conf_syst}
\end{equation}
One can calculate the likelihoods for the data,
perform the integral over nuisance parameters
and then normalise to calculate the probability density of $N_\text{sig}$
...
This step is known as marginalisation.

Using the posterior calculated from Equation~\ref{eq:lim-conf_syst},
the 95\% credibility level upper limit of $N_\text{sig}$, denoted by $N_\text{sig}^{\,\text{up}}\,$,
is calculated using the expression
\begin{equation}
\int_0^{N_\text{sig}^{\,\text{up}}} P(N_\text{sig} \mid D)~=~0.95
\end{equation}
Under the assumption of $H_{N_\text{sig}}$ there is a 95\% probability that the value of $N_\text{sig}$ lies
within the credibility interval defined as \mbox{$0 \leq N_\text{sig} < N_\text{sig}^{\,\text{up}}$}.
Therefore, any model under the hypothesis $H_{N_\text{sig}}$ that predicts a $N_\text{sig}$
value above the upper limit, $N_\text{sig}^{\,\text{up}}$, is excluded at the 95\% credibility level.
...
which is related to the parameter $N_\text{sig}$ used in the limit setting description
\footnote{
  Specifically $N_\text{sig}$=$L\,\text{x}\,\sigma\,\text{x}\,\mathit{A}\,\text{x}\,\epsilon$, where $L$ is the luminosity.
}.
...
The Bayesian approach defines a credibility interval using the probability (or degree of belief) in a hypothesis given the observed data \mbox{( $P(N_\text{sig} \mid D)$ )}.
On the other hand, the frequentist approach calculates the probability (or fraction of trials)
of obtaining the data assuming a given signal model is true \mbox{( $P(D \mid N_\text{sig})$ )} and rejects models that produce a low probability.
...
Section~\ref{sec:bkg-full_swift} described the SWiFt background estimation procedure in the search phase of the \lm{} data-set, 
for clarity this will be referred to as the \textit{`nominal SWiFt background estimation'} in this section.
The nominal SWiFt background estimation is model independant as there is no assumption of any signal models in the procedure.
...
This was particularly notable for a SSM $Z'$ boson with a generated mass of 600~GeV,
...
A window half-width of 16 and the 5 parameter fit function are chosen to match the configuration used in the search phase results shown in Section~\ref{sec:bkg-full_results}.
The normalisation of the signal template and the parameters of the background function are
chosen to maximise the likelihood (defined in Eqn.~\ref{eqn:lim-like}), the signal normalisation is required to be greater or equal to zero.

The signal plus background fit provides an estimate of the signal and background components in the SWiFt window considered,
however the framework used for limit setting requires a background estimation for the full mass range considered.
To extend the fit to the full range, the signal template, normalised by the signal plus background fit, is subtracted from the data.
The SWiFt background estimation procedure is then performed to the signal subtracted data,
using the 5 parameter dijet fit function and a window half-width of 16, which is again chosen to match the configuration in the search phase.
The resulting background estimation is the \textit{`signal subtracted background estimation'}
and is used as the background template in the limit setting phase.
A signal subtracted background estimation is created for each generated mass point for each signal.

The signal subtracted background is used as it provides a simple method of extending the signal plus background fit to create a background estimate for the full mass range. 
Futhermore, the signal subtracted background estimate is used by the inclusive dijet search on the
combinded 2015 and 2016 data-set at ATLAS~\cite{dijet-mori17_paper}.
It is benefitial for related analyses at ATLAS to utilise similar background estimation techniques as this
increases reliability as the technique is independantly validated multiple times,
...
In Figure~\ref{fig:lim-lowmass_ssb_test}(a) the signal subtracted background estimate~(blue) and nominal SWiFt background~(red) estimations
...
\begin{figure}[!ht]
  ...
  \caption[The nominal SWiFt background and signal subtracted background (SSB) estimations
    ...
    The nominal SWiFt background estimation procedure is model used in the search phase,
...
For all generated mass points, including those not shown in the figure,
...
Therefore, the nominal SWiFt background estimate used in the search phase and the 
signal subtracted background estimate used in the limit setting phase are consistent.
...
95\% credibility level upper limits set on $\sigma\,\text{x}\,\mathit{A}\,\text{x}\,\epsilon$
of a $Z'$ boson as a function of generated mass.
...
The change in gradiant of the theoretical predicition lines at $m_{Z'} =$ 800~GeV
is a result of a reduction in the acceptance of the $Z'$ boson for $m_{Z'} <$ 800~GeV
caused by the requirement that the dijet mass is greater than 566~GeV.
This effect is discussed further in Section~\ref{sec:evt-sel-acc}
and can be seen clearly in Figure~\ref{fig:evt-lm_acc}.
...
Additionally it is anticipated the DM $Z'$ boson will be excluded in the generated mass range
...
The Gaussian shapes in truth mass are centred on a range of generated masses and the width of the considered Gaussians are
15\%, 10\%, 7\%, 5\%, 3\% and 0\% of the generated mass;
...
majority of the signal events are contained within the mass range probed by this analysis for all signal widths.
...
the expected limits are shown by the dotted lines and associated 1~and~2~$\sigma$ uncertainty bands are shown in green and yellow.
...
for a generic Gaussian signal ranging from 0.05 to 0.003 pb in the mass range 0.65~to~1.4~TeV.
...
\begin{figure}[!ht]
  ...
  {95\% credibility observed upper limits
    ...
    The signal templates are Gaussian in generated mass with

...
Specifically, one can estimate the degree of belief that a signal model is true given the di-$b$-jet spectra that have been observed.
...
In this Chapter
Section~\ref{sec:lim-strat} will describe the limit setting strategy used and
...
the QCD background, which is modelled by the background estimation procedures described in the previous chapter,
and some new physics process 
which produces $\mu$ events according to a dijet mass signal template.
...
which in this case is one of the observed di-$b$-jet spectra.
For the hypothesis, $H_\mu$, the probability of producing the data is known as the likelihood.
If, in each dijet mass bin, labelled by the index $i$, the model predicts
$s_i(\mu)$ signal events, $b_i$ background events and $n_i$ events were observed in data then,
by only considering statistical uncertainties, the likelihood for a given value of $\mu$ is given by
\begin{equation}
...
\end{equation}
Therefore there is a 95\% probability that the parameter $\mu$ lies
within the interval \mbox{$0 \leq \mu < \mu_{up}$}.
Any model under the hypothesis $H_{\mu}$ that predicts a $\mu$
value outside of this interval is said to be excluded at the 95\% credibility level.
...
The first is the observed limit, which is the limit using the observed di-$b$-spectra as $D$, as described above.
The second is the expected limit under the assumption that there is no signal in the di-$b$-jet spectrum,
the expected limit shows the sensitivity of the limit setting phase.
...
of obtaining the data assuming a given signal model is true \mbox{( $P(D \mid \mu)$ )} and rejects models that produce a low probability~\cite{lim-cowan}.
...
but it is important that one states clearly which approach is being \mbox{taken \footnote{As a side note the BumpHunter $p$-value uses the frequentist approach to calculate a $p$-value.}.}

\section{Description of Systematic Uncertainties}
\label{sec:lim-syst}

The sources of systematic uncertainty in the di-$b$-jet analysis are grouped into two categories~\cite{dibjet-ichep_conf}.
The first group are uncertainties on the dijet mass signal templates used in the limit setting procedure, which have been produced in Monte-Carlo simulations.
...
\item\textbf{Jet Energy Scale, Jet Energy Resolution  and $b$-Jet Energy Scale} \hspace{1mm} (\textit{Signal}):\\
  ...
  which cause an uncertainty on the width of the dijet mass signal templates.
...
\item \textbf{Fit Function Parameters} \hspace{1mm} (\textit{Background}):\\
  ...
  The root mean square (rms) of the difference between the nominal background estimate on data to those from the pseudo-experiments is
...
These two mass points are tested as the signal injection studies, presented in Section~\ref{sec:bkg-full_signalInj}, showed that the search phase
would not produce a significant observation of a SSM $Z'$ boson at these simulated mass points. 
...
\begin{figure}[!ht]
  ...
  \end{center}
  \vspace{-1mm}
  \caption{The nominal SWiFt background (red) and signal subtracted background (SSB) (blue) estimations
...
as uncertainties are found to have a small effect on the upper limit relative to the resolution uncertainties,
as these can significantly affect the width of the dijet mass signal template.

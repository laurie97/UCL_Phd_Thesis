...
A window half-width of 16 and the 5 parameter fit function are used as is done in the search phase results shown in Section~\ref{sec:bkg-full_results}.
...
using the 5 parameter dijet fit function and a window half-width of 16, which is again chosen to match the search phase results.
...
Futhermore, the signal subtracted background is also chosen as it has been used by a previous inclusive dijet search at ATLAS~\cite{dijet-mori17_paper}.
...
there is a reduced gradient of the predicition for $m_{Z'} <$800~GeV is due to the loss in acceptance at low mass due to the requirement that the dijet mass is greater than 566~GeV,
as is observed in Figure~\ref{fig:evt-lm_acc}.
...
This is because the jet energy uncertainties are uncertainties on the dijet mass of a simulated event and

...
In Chapter~\ref{sec:bkg} it was shown that there is no evidence of new physics in the dijet mass spectra of the di-$b$-jet events.
...
To set limits on a particular model, one considers the hypothesis that the di-$b$-jet events
...
and the BSM physics model is desribed by a dijet mass signal template, normalised such that $\mu$ signal di-$b$-jet events are produced.
...
The first is the observed limit, which is set using the observed dijet mass spectra for $D$, as described above.
...
The first group are uncertainties on the dijet mass signal templates used in the limit setting procedure, which are produced using Monte-Carlo simulations.

...
and particle identification and measurement (such as the electrons and muons as described in Section~\ref{sec:obj-electrons} and~\ref{sec:obj-muons}).
...
As with any part of a measurement, the process of $b$-tagging must be calibrated in data.
$b$-tagging calibration~\cite{obj-bjets_calib} a pure sample of $b$-jets is found using a di-lepton $t\bar{t}$,
using similar selection to the analysis described in Section~\ref{sec:trig-bjet_eff}.
...
\end{equation}
where $b$-tagged means above the cut on MV2 output for that operating point.
...
\begin{equation}
 SF_{\text{bTag}} = \frac{\epsilon_{\text{bTag}}^{\text{Data}}}{\epsilon_{\text{bTag}}^{\text{MC}}}
\end{equation}

\textbf{LM Fix} Extrapolation to high pT....

Uncertainties are derived for the scale factor,
the dominant of which account for uncertainties in $t\bar{t}$ modelling in simulation
and the procedure to high-$p_T$ \textbf{LM Fix}

Figure ?? shows the data/MC scale factor measured in 2015 and 2016 data, as a function of jet $p_T$ and $\eta$.
The scale factor is close to 1 everywhere, showing the $b$-tagging is well generally well-modelled in simulation. \textbf{LM Fix}

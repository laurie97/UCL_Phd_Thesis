...
The previous chapter described that the ATLAS detector consists of many sub-detectors
that each provide a set of measurements
in the form of, for example, hits in the Inner Detector or energy deposits in the calorimeter.
These measurements from the sub-detectors are combined to reconstruct the physics objects required to study the hard-scatter process, 
...
\item\textbf{Track Candidates:}
  Initial track candidates are formed using only hits from the IBL, pixel detector and SCT because they have a higher precision and granularity.
...
\item\textbf{Track selection / Ambiguity resolving:}
  ...
  the the compatibility of hits to the track candidate and the \pT{} and $\eta$ of the track.
  ...
  Full details of the track-selection algorithm are found in~\cite{obj-tracks_TIDE}.\vspace{0.5em}
...
%Tracks ar
The end result of this process are tracks.
The momentum and sign of charge of the charged particles can be determined from the sign and magnitude of tracks' curvature
caused by the solenoid magnet.

\subsection{Primary Vertex Identification}
\label{sec:obj-tracks_pv}

As described in Section~\ref{sec:det-LHC} the LHC collides bunches of protons leading to multiple collisions per bunch crossing;
...
The hard-scatter primary vertex, the point at which the collision of interest occurred,
is defined as the primary vertex with the highest sum of the squared transverse momenta ($\sum {\pT}^2$) of associated tracks.


\section{Jets}
\label{sec:obj-jets}

Quarks or gluons that are created in a proton-proton scatter event
...
\FloatBarrier
This section contains a description the jet building procedure utilised by ATLAS
...
%firstly topoclusters are formed (Section~\ref{sec:obj-jets_topo}),
%then jets are formed from topoclusters using reclustering algorithms (Section~\ref{sec:obj-jets_reco})
%and then the jets are calibrated and relevant jet energy uncertainties are derived (Section~\ref{sec:obj-jets_calib}~--~\ref{sec:obj-jets_uncert}).
...
%a technique that has been useful in dense environments
...
The calorimeter cells can be from either the EM or hadronic calorimeter systems which have a granularity described in Section~\ref{sec:det-calo}.
...
\begin{figure}[!ht]
  \begin{center}
    \includegraphics[width=0.4\linewidth, angle=0]{figs/Objects/topo_schem.png}
  \end{center}
...
In the next step of jet building, jet reconstruction algorithms are employed to form jets from
...
The three algorithms use a set of four-momenta (clusters), which are initially the topoclusters described above.
...
\begin{equation}
  d_{ij} = \text{min}[(p_{ Ti})^a, (p_{ Tj})^a]  \left(\frac{\Delta  R_{ij}}{R}\right) ^2, \hspace{2cm}  d_{iB} = (p_{Ti})^a  \label{dij}
\end{equation}
%\begin{equation}
%  d_{ij} = \text{min}
...
This is because the anti-$k_T$ algorithm provides regular jet shapes around the centre of the jet because
the algorithm reconstructs the high-\pT{} core of the jets first and then adds in the lower \pT{} suburbs. % in later steps.
Figure~\ref{fig:obj-jets_reco_shapes} shows the jets built by the %Cambridge-Aachen,
...
\item\textbf{Jet energy scale}:
  ...
  Therefore energy measurements of hadronic objects must be corrected for using a jet energy scale correction. \vspace{0.5em}
...
\item\textbf{Pile-up}:
  ...
  Objects from pile-up collisions can be included in the jet reconstruction and hence effect the jet energy measurement.
...
\end{itemize}
\vspace{-0.5em}
A calibration procedure is performed to correct the energy of a jet~\cite{obj-jets_calib_run2}.

For a calibration one must decided what to correct with respect to.
...
\item\textbf{Absolute JES Correction}:
  This step corrects the jet four-momentum from the EM-scale, at which they were initially built,
  to the hadronic-scale, which is defined in terms of the truth jets in simulation.
  This correction is derived using truth jets and reconstructed detector-level jets in dijet Monte-Carlo events.\vspace{0.3em}
...
\item\textbf{In-situ calibration}:
  ...
  One can then calculate a double ratio, $R$, which is applied as a correction to jet \pT{} in data only.
  \begin{equation}
...
This type of jet is known as `anti-$k_T$ R=0.4 EM+JES jet',
...
The increased uncertainty in the region $2 < |\eta| < 2.6$ is caused by uncertainties in
modelling of the detector response of the EM end-cap calorimeters.
%required to account localised data/simulation differences observed in that region,
%which is caused by the modelling of the detector response .
%the modelling of detector response in the transition region between different layers of the electro-magnetic end-cap calorimeters.

\begin{figure}[!htb]
  \begin{center}
    \captionsetup[subfigure]{aboveskip=0pt,justification=centering}
    \subcaptionbox{Jet-\pT{}} {\includegraphics[width=0.42\linewidth, angle=0]{figs/Objects/jets_uncert_JES_pt.png} }
    \subcaptionbox{Jet-\eta}{\includegraphics[width=0.42\linewidth, angle=0]{figs/Objects/jets_uncert_JES_eta.png} }
  \end{center}
 \vspace{-1em}
...
\begin{figure}[!htb]
  \begin{center}      
    \captionsetup[subfigure]{aboveskip=0pt,justification=centering}
    \subcaptionbox{Jet-\pT{}} {\includegraphics[width=0.42\linewidth, angle=0]{figs/Objects/jets_uncert_JER_pt.pdf} }
    \subcaptionbox{Jet-\eta}{\includegraphics[width=0.42\linewidth, angle=0]{figs/Objects/jets_uncert_JER_eta.pdf} }
 \end{center}
 \vspace{-1em}
...
There are three separate categories f hadronic jets  based on the flavour of the constituent quarks.
...
and finally light jets comprise of only light hadrons (formed of $u$, $d$ and $s$ quarks).
...
Weakly decaying $b$-hadrons produced at the LHC have an average lifetime of $\sim$\SI{1.6}{\pico\second} ~\cite{obj-bjets_PDG}
 \footnote{\ There are also excited $b$-hadron states decay rapidly to other $b$-hadrons through the strong force.}.
...
For $b$-tagging tracks are associated to jets if there is a small angular separation $\Delta R$ between the two objects.
...
The discriminating variables are then combined in a multi-variate algorithm which is described in Section~\ref{sec:obj-bjets_MV2}.
  ...
  \begin{center}
    ...
    \caption[An illustration of how the three base flavour tagging algorithms are combined in the MV2 algorithm.]
            {An illustration of how the three base flavour tagging algorithms are combined in the MV2 algorithm.
              The figure shows that the discriminating variables from SV1 and Jet Fitter are combined with the likelihood outputs from IP2D and IP3D.}
...
\begin{figure}[!ht]
  \begin{center}
    \captionsetup[subfigure]{aboveskip=0pt,justification=centering}
    \subcaptionbox{$b$-jet efficiency \\against light-jet rejection} {\includegraphics[width=0.48\linewidth, angle=0]{figs/Objects/bjets_perf_light}}
    \subcaptionbox{$b$-jet efficiency \\against $c$-jet rejection}  { \includegraphics[width=0.48\linewidth, angle=0]{figs/Objects/bjets_perf_charm}}
  \end{center}
...
Table~\ref{tab:obj-MV2_WPs} shows the fixed cut operating points used at ATLAS with a given cut on MV2c10 output;
...
meaning that the signature is diluted.
...
To illustrate this trend, Figure~\ref{fig:obj-bjets_perf_pt} $b$-tagging efficiency for $b$-jets, $c$-jets and light-jets in $t\bar{t}$ simulation as a function of jet-\pT{}
...
$b$-tagging calibration uses di-lepton $t\bar{t}$ events which, as described in Section~\ref{sec:theo-qcd_ttbar},
have a distinctive signature and provide a pure sample of $b$-jet~\cite{obj-bjets_calib_tech,obj-bjets_calib_plots}.
From the pure $b$-jet sample one can perform a likelihood fit to extract the $b$-jet efficiency, $\epsilon_{\text{bTag}}$, which is defined as:
\begin{equation}
...
\begin{figure}[!ht]
  \begin{center}
    \includegraphics[width=0.45\linewidth, angle=0]{figs/Objects/bjets_calib_pt.pdf} 
  \end{center}
  \vspace{-1em}
...
To derive an uncertainty for the high-\pT{} extrapolation; $\epsilon_{\text{bTag}}$ is measured in Monte-Carlo simulations of high-\pT{} $b$-jets  
when variables known to affect the performance of $b$-tagging are varied to represent our uncertainty of $b$-tagging modelling at high-\pT{}~\cite{obj-bjets_calib_highPt}.
The dominant extrapolating uncertainty arises for variations of the impact parameter resolution.
...
jet energy correction applied  hadronic jets the associated uncertainty jet energy scale uncertainty.
...
The $b$JES uncertainty is validated by comparing jet energy measurements to independent well calibrated objects, in this case tracks.
It is found that the energy of $b$-jets is consistent with the energy of inclusive jets within the $b$JES uncertainties considered,
such that no additional $b$JES correction is required.
...
hence muons are identified at ATLAS using hits in the Muon Spectrometer (MS), which was described in Section~\ref{sec:det-MS}.
Two of the techniques used for muon reconstruction are combined muons and extrapolated muons.
For both techniques initially trajectories are reconstructed from hits in the MS, which are known as muon tracks.
Combined muons are then reconstructed by extrapolating muon tracks inwards to match tracks in the ID (described in Section~\ref{sec:obj-tracks}),
...
The four-momentum of the reconstructed muons can be constructed from the direction and curvature of the muon tracks.

The muon identification working points are \textit{Loose}, \textit{Medium}, \textit{Tight}, and \textit{High-\pT{}}.
Medium muons, used in Section~\ref{sec:trig-bjet_eff}, use combined and extrapolated muons that pass a quality criteria based on
...
%\item\textbf{Taus:}
%  Taus decaying hadronically are identified and reconstructed using narrow calorimeter jets
%  associated to a topologies of tracks that match their known decay chain~\cite{obj-taus}.\vspace{0.5em}
\textit{`Missing Transverse Momentum''} (MET) is the negative sum of the momenta of all reconstructed physics objects in an event~\cite{obj-met}.

...
A GRL is applied to all data-sets considered in this anlysis.
...
\item\textbf{Full16\_LowMass}: \\
  ...
  The \verb|Full16_LowMass| uses a $b$-jet trigger aware GRL which additionaly
...
\item\textbf{Background: QCD Di-jet}: \\
  ...
  However, a simulated QCD dijet sample is also used in this analysis
  for background studies and background modelling validation.
  \end{itemize}

\noindent
Before describing the signal models used it is useful to clearly differentiate between the two defintitions of mass used in this analysis.
...
\footnote{Un-prescaled means that the trigger accepts every event passing the trigger selection criteria};
...
Finally, the invariant mass of the two leading jets, \mjj, is required to above a threshold value.
...
\footnote{This is the amount of data collected when the studies where performed}.
...
To further understand the effect of $b$-tagging is this analysis the flavour composition of the background is studied.
...
These events are pass both the \verb|Summer16+15| and \verb|Full16+15_HighMass| data-set event selection.

...
%Chapters~\ref{sec:evt},~\ref{sec:bkg}~and~\ref{sec:lim}
...
\item\textbf{Di-$b$-Jet Event Selection:} (\textit{Chapter~\ref{sec:evt}})\\
  ...
  Chapter~\ref{sec:evt} will provide details on %the data-sets considered and
...
\item\textbf{Search Phase:} (\textit{Chapter~\ref{sec:bkg}})\\
  ...
  \begin{figure}[!hbt]
  \begin{center}
    \includegraphics[width=0.8\linewidth, angle=0]{figs/Dibjet/Gen/dijet_schem.pdf}
  \end{center}
  \vspace{-3mm}
...
The background and signal models are utilised in optimisation and validation studies
of the event selection and search phase of the analyses (presented in Chapters~\ref{sec:evt}~and~\ref{sec:bkg} respectively).
...
Firstly, the event selection is constructed maximise sensitivity to signal:
...
%\label{sec:evt-sel}
%
%The overall aim when designing the di-$b$-jet event selection is two-fold.
...
Furhtermore, to visualise events that pass the event selection,

...
\item\textbf{Search Phase:} (\textit{Chapter~\ref{sec:bkg}})
  ...
  \begin{figure}[!hbt]
  \begin{center}
    \includegraphics[width=0.75\linewidth, angle=0]{figs/Dibjet/Gen/dijet_schem.pdf}
  \end{center}
  \vspace{-3mm}
...
\item\textbf{Signal: $Z'$ Boson}:  
  ...
  results are easier to interpret for other signal models decaying to pairs of $b$-quarks. \vspace{1em}

  
  Furthermore, it has been shown that for a $Z'$ boson model the cross-sections can increase by up to 30\%
  ...
  and are found to be between 1.2 and 1.3 depending on the generated mass~\footnote{
  ~It is important to note that the correction for NLO effects only relates to the normalisation of the dijet mass signal template and does not change its shape.
   As a result, this correction does not affect the expected and observed upper limits set on the $\sigma\,\text{x}\,\mathit{A}\,\text{x}\,\epsilon$ of the SSM and leptophobic $Z'$ boson
   but will improve the mass limits set on the SSM and leptophobic $Z'$ boson; both limits are presented and discussed in Chapter~\ref{sec:lim}.
  }.
